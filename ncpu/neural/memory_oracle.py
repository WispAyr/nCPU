...
class DispatcherTelemetry:
    ...
    def print_summary(self, title: str = "INTELLIGENT DISPATCHER TELEMETRY"):
        ...
        logger.info("\n  MEMORY ORACLE:")
        logger.info(f"    Total accesses:      {self.oracle_total_accesses:>12,}")
        logger.info(f"    Predictions made:    {self.oracle_predictions:>12,}")
        logger.info(f"    Prefetch hits:       {self.oracle_hits:>12,}")
        logger.info(f"    Hit rate:            {self.oracle_hit_rate:>11.1%}")
        logger.info(f"    LSTM predictions:    {self.oracle_lstm_predictions:>12,}")
        logger.info(f"    Stride detections:   {self.oracle_stride_detections:>12,}")

        logger.info("\n  SEMANTIC DISPATCHER:")
        logger.info(f"    Patterns detected:   {self.dispatcher_patterns_detected:>12,}")
        logger.info(f"    Instructions saved:  {self.dispatcher_instructions_saved:>12,}")
        logger.info(f"    Bytes accelerated:   {self.dispatcher_bytes_accelerated:>12,}")
        logger.info(f"    Try dispatch calls:  {self.dispatcher_try_calls:>12,}")
        logger.info(f"    Detection rate:      {self.dispatcher_detection_rate:>11.1%}")

        logger.info("\n  SAFETY:")
        logger.info(f"    Bounds violations:   {self.safety_bounds_violations:>12,}")
        logger.info(f"    Null page rejects:   {self.safety_null_page_rejections:>12,}")
        logger.info(f"    Overflow rejects:    {self.safety_overflow_rejections:>12,}")
        logger.info(f"    Prefetch rejects:    {self.safety_prefetch_rejections:>12,}")

        logger.info("\n  ADAPTIVE THRESHOLD:")
        logger.info(f"    Current threshold:   {self.adaptive_current_threshold:>11.2f}")
        logger.info(f"    Adaptations made:    {self.adaptive_adaptations:>12,}")
        logger.info(f"    Window hit rate:     {self.adaptive_window_hit_rate:>11.1%}")

        logger.info("\n  TRAINED LSTM:")
        logger.info(f"    Model loaded:        {'Yes' if self.trained_model_loaded else 'No':>12}")
        if self.trained_model_loaded:
            logger.info(f"    Pattern accuracy:    {self.trained_model_pattern_accuracy:>11.1%}")
            logger.info(f"    Current pattern:     {self.trained_current_pattern:>12}")
            logger.info(f"    Pattern confidence:  {self.trained_pattern_confidence:>11.1%}")
...
class MemoryOracle:
    ...
    def _load_trained_model(self):
        ...
        if model_path.exists():
            try:
                ...
                logger.info(f"   Memory Oracle: Trained LSTM ACTIVE (pattern acc: {pattern_acc:.1%})")

            except Exception as e:
                self.trained_model_loaded = False
                self.trained_lstm = None
                logger.warning(f"   Memory Oracle: Could not load trained model: {e}")
        else:
            logger.info("   Memory Oracle: No trained model found (using stride detection)")
    ...
    def print_stats(self):
        ...
        logger.info(f"  Prefetch hits:     {stats['hits']:,}")
        logger.info(f"  Hit rate:          {stats['hit_rate']:.1%}")
        logger.info(f"  Prefetches issued: {stats['prefetches_issued']:,}")
        logger.info(f"  Bytes prefetched:  {stats['bytes_prefetched']:,}")
        logger.info(f"  Trained LSTM:      {'Active' if stats['trained_model_loaded'] else 'Not loaded'}")
        if stats['trained_model_loaded']:
            logger.info(f"  Pattern accuracy:  {stats.get('trained_pattern_accuracy', 0):.1%}")
            logger.info(f"  Current pattern:   {stats.get('current_pattern', 'unknown')} ({stats.get('pattern_confidence', 0):.1%})")
        logger.info(f"{'─'*50}")
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("  MEMORY ORACLE TEST")
    logger.info("=" * 60)
    ...
    logger.info(f"  Pattern detected: {pattern} (confidence: {conf:.2%})")

    # Predict next addresses
    predictions, confidences = oracle.predictor.predict_next(base + 100 * 8)
    logger.info(f"  Predictions: {len(predictions)} addresses")
    if predictions:
        expected_next = base + 101 * 8
        error = abs(predictions[0] - expected_next)
        logger.info(f"  First prediction error: {error} bytes")
    ...
    logger.info(f"  Detected stride: {stride} (expected: {struct_size})")
    ...
    if result:
        logger.info(f"  Pattern: {result[0]}")
        logger.info(f"  Params: {result[1]}")
    else:
