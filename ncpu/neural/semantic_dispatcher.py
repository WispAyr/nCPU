...
class SemanticDispatcher:
    ...
    def print_stats(self):
        ...
        logger.info(f"  Instructions accelerated: {self.stats['instructions_accelerated']:,}")
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("  SEMANTIC DISPATCHER TEST")
    logger.info("=" * 60)
    ...
    logger.info(f"  Handled: {result.handled}")
    logger.info(f"  Instructions skipped: {result.instructions_skipped}")
    logger.info(f"  Memory[0x1000]: {memory[0x1000].item()} (expected: 0x42)")
    ...
    logger.info(f"  Handled: {result.handled}")
    logger.info(f"  Instructions skipped: {result.instructions_skipped}")
    logger.info(f"  Memory[0x3050]: {memory[0x3050].item()} (expected: 50)")
    ...
    logger.info(f"  Handled: {result.handled}")
    logger.info(f"  Length: {result.result_value} (expected: {len(test_str)})")
    ...
    logger.info(f"  Handled: {result.handled}")
    logger.info(f"  Result: {result.result_value} (expected: 0 for equal)")
